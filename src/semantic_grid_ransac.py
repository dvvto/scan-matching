...
class Feature2DGlobalRegistrationPipeline:
    ...
    def get_fitness(self, ref_grid: np.ndarray, cand_grid: np.ndarray, transform: np.ndarray) -> float:
        ...
        return good_match / (good_match + bad_match) * iou ** 0.25#, ref_wall_mask, ref_floor_mask, cand_wall_mask, cand_floor_maskz

    def extract_and_match_features(self, img_ref: np.ndarray, img_cand: np.ndarray, verbose: bool = False):
        # Extract features
    ...
    def compute_cluster_centroids(self, grid_semantic):
        cluster_centroids = []
        for i in range(3, len(grid_semantic)):
            cluster_centroids.append([])
            points_ij = grid_semantic[i]
            if points_ij.shape[0] < 5:
                continue
            clustering = DBSCAN(eps=5, min_samples=1).fit(points_ij)
            clusters = clustering.labels_
            for cluster_id in range(clusters.max() + 1):
                cluster_points = points_ij[clusters == cluster_id]
                if cluster_points.shape[0] < 5:
                    continue
                centroid = cluster_points.mean(axis=0)
                cluster_centroids[-1].append(centroid)
        return cluster_centroids

    def remove_outliers(self, src_pts: np.ndarray, dst_pts: np.ndarray, 
                        verbose: bool = False,
                        starting_transform: Union[Tuple, None] = None):
        # Remove outliers            
        for i in range(len(self.outlier_thresholds)):
            if len(src_pts) < self.min_matches:
                ...
                self.cnt += 1
                return None
            if i == 0 and starting_transform is not None:
                rot_angle, trans_j, trans_i = starting_transform
            else:
    ...
    def infer(self, ref_grid: Tensor, ref_grid_semantic: np.ndarray, 
              cand_grid: Tensor, cand_grid_semantic: np.ndarray,
              verbose: bool = False) -> Tuple[Union[np.ndarray, None], Union[float, None]]:
        t1 = time()
        ...
        t2 = time()
        if verbose:
            print('Image preprocessing time:', t2 - t1)
        
        src_pts, dst_pts = self.extract_and_match_features(img_ref, img_cand, verbose=verbose)
        t3 = time()
        if verbose:
            print('Feature matching time:', t3 - t2)

        # Try to remove outliers (old method, without semantic)
        transform = self.remove_outliers(src_pts, dst_pts, verbose=verbose, starting_transform=None)
        t4 = time()
        if verbose:
            print('Outlier removing time:', t4 - t3)
        if transform is not None:
            fitness = self.get_fitness(ref_grid.cpu().numpy(), cand_grid.cpu().numpy(), transform)
            if fitness > self.fitness_threshold:
                return transform, fitness

        # If match not found, try with semantic
        cluster_centroids_ref = self.compute_cluster_centroids(ref_grid_semantic)
        cluster_centroids_cand = self.compute_cluster_centroids(cand_grid_semantic)
        t5 = time()
        if verbose:
            print('Clustering time:', t5 - t4)
        cluster_matches = []
        for i in range(len(cluster_centroids_ref)):
            for cluster_ref in cluster_centroids_ref[i]:
                for cluster_cand in cluster_centroids_cand[i]:
                    cluster_matches.append([cluster_ref, cluster_cand])
        if verbose:
            print('Found {} pairwise cluster matches'.format(len(cluster_matches)))
        if len(cluster_matches) < 2:
            return None, 0
        cluster_matches = np.array(cluster_matches)
        index = np.arange(len(cluster_matches))
        # Run RANSAC on cluster centroid matches
        match_pairs = []
        if len(cluster_matches) ** 2 < self.ransac_iterations:
            for i in range(len(cluster_matches)):
                for j in range(i + 1, len(cluster_matches)):
                    match_pairs.append([i, j])
        else:
            for i in range(self.ransac_iterations):
                match_pairs.append(np.random.choice(index, size=2))
        best_fitness = 0
        best_transform = None
        for i, j in match_pairs:
            match1, match2 = cluster_matches[i], cluster_matches[j]
            dst1 = np.sqrt(np.sum((match1[0] - match2[0]) ** 2))
            dst2 = np.sqrt(np.sum((match1[1] - match2[1]) ** 2))
            if abs(dst1 - dst2) > self.distance_threshold:
                continue
            if verbose:
                print(dst1, dst2, match1, match2)
            src_pts_cluster = np.array([match1[0], match2[0]])
            src_pts_cluster = src_pts_cluster[:, np.newaxis, :]
            dst_pts_cluster = np.array([match1[1], match2[1]])
            dst_pts_cluster = dst_pts_cluster[:, np.newaxis, :]
            point_pairs = np.concatenate([src_pts_cluster, dst_pts_cluster], axis=1)
            transform = self.remove_outliers(src_pts.copy(), dst_pts.copy(), verbose=verbose, 
                                             starting_transform=self._point_based_matching(point_pairs))
            if transform is not None:
                fitness = self.get_fitness(ref_grid.cpu().numpy(), cand_grid.cpu().numpy(), transform)
