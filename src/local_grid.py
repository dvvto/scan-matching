...
class LocalGrid:
    def __init__(self, 
                 ...
                 semantic_probability_threshold=0.2,
                 curbs_attenuation=0.99,
                 layer_names=['curbs', 'occupancy', 'semantic', 'height_map', 'density_map', 'density_map_cur'],
                 save_dir=None):
        ...
        self.semantic_probability_threshold = semantic_probability_threshold
        ...
        for layer_name in self.layer_names:
            ...
            if layer_name == 'semantic':
                self.layers[layer_name] = np.zeros((self.grid_size, self.grid_size, 21), dtype=dtype)
            else:
    ...
    def update_from_cloud_and_transform(self, points_xyz, probabilities,
                                        x=0, y=0, theta=0):
        self.transform(x, y, theta)
        points_xyz = np.hstack([points_xyz, probabilities])
        ...
        probabilities = points_xyz_obstacles[:, 3:]
        points_xyz_obstacles = points_xyz_obstacles[:, :3]
        ...
        self.layers['semantic'] = []
        for cls in range(n_classes):
            points_ij_of_class = points_ij_obstacles[probabilities[:, cls] > self.semantic_probability_threshold]
            points_ij_of_class = np.unique(points_ij_of_class, axis=0)
            self.layers['semantic'].append(points_ij_of_class)
        # Fill density map
    ...
    def transform(self, x, y, theta):
        for layer_name in self.layer_names:
            if layer_name == 'semantic':
                for channel in range(self.layers[layer_name].shape[2]):
                    self.layers[layer_name][:, :, channel] = self.get_transformed_grid(self.layers[layer_name][:, :, channel], x, y, theta)
            else:
