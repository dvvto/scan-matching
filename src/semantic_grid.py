...
class Feature2DGlobalRegistrationPipeline:
    ...
    def infer(self, ref_grid: Tensor, ref_grid_semantic: Tensor, 
              cand_grid: Tensor, cand_grid_semantic: Tensor,
    ...
    # def infer(self, img_ref, img_cand, verbose=True):
        ...
        # t1 = time.time()
        ref_grid_numpy = ref_grid_semantic.cpu().numpy()
        cand_grid_numpy = cand_grid_semantic.cpu().numpy()
        all_matches = []
        kp_ref_all = []
        kp_cand_all = []
        for channel in range(ref_grid_numpy.shape[2]):
            img_ref = (ref_grid_numpy[:, :, channel] == 1)
            ...
            img_cand = (cand_grid_numpy[:, :, channel] == 1)
            ...
            kp_ref_all.append(kp_ref)
            kp_cand_all.append(kp_cand)
            ...
            if self.detector_type == 'SIFT':
                ...
                if des_ref is None or des_cand is None or len(des_ref) < 2 or len(des_cand) < 2:
                    ...
                    continue
                matches = flann.knnMatch(des_ref, des_cand, k=2)
                matches = [(channel, x[0], x[1]) for x in matches if len(x) == 2]
            else:
        ...
        # elif self.detector_type == 'HarrisWithDistance':
            ...
            #     matches.append((m, n))
        if verbose:
            print('Found {} matches'.format(len(all_matches)))
        # t4 = time.time()
        ...
        if self.detector_type == 'HarrisWithDistance':
            ...
            matching_threshold = 0.8
        for i,(c,m,n) in enumerate(all_matches):
        ...
        src_pts = np.float32([ kp_ref_all[c][m.queryIdx].pt for c, m in good ]).reshape(-1,1,2)
        dst_pts = np.float32([ kp_cand_all[c][m.trainIdx].pt for c, m in good ]).reshape(-1,1,2)
        ...
        return transform, self.get_fitness(ref_grid.cpu().numpy(), cand_grid.cpu().numpy(), transform)